...
public partial class SetupWizard : Window {
    ...
    private readonly List<Canvas> Windows = new();
    ...
    private void Prepare1() {
        var currentCanvas = this.FindControl<Canvas>("hrtypepanel");
        Windows.Add(currentCanvas);
        List<RadioButton> rbs = new();
        RadioButton selected = null;
        var sp1 = this.FindControl<StackPanel>("sp11");
        var sp2 = this.FindControl<StackPanel>("sp12");
        foreach (var sp1Child in sp1.Children) {
            var rb = (RadioButton)sp1Child;
            rb.Checked += (sender, args) => selected = rb;
            rbs.Add(rb);
        }

        foreach (var sp2Child in sp2.Children) {
            var rb = (RadioButton)sp2Child;
            rb.Checked += (sender, args) => selected = rb;
            rbs.Add(rb);
        }
            ...
            if (selected != null) {
                if (hrtypes.ContainsKey(selected.Name)) {
                    hrtypes[selected.Name].ShowExtra(vals => {
                        ...
                        foreach (var hrTypeExtraInfo in vals) {
    ...
    private void Prepare2() {
        var currentCanvas = this.FindControl<Canvas>("endpointpanel");
        Windows.Add(currentCanvas);
        var se = this.FindControl<Canvas>("showextra2");
        var ad = false;
        ...
        this.FindControl<Button>("continue2").Command = new OnButtonClick(() => {
            if (ad) {
                var ipb = this.FindControl<TextBox>("ip");
                var spb = this.FindControl<TextBox>("sendport");
                var lpb = this.FindControl<TextBox>("listenport");
    ...
    private record HRTypeSelector(string Name) {
        ...
        public void ShowExtra(Action<List<HRTypeExtraInfo>> onDone) {
            ...
            foreach (var hrTypeExtraInfo in ExtraInfos) {
                var title = new Label {
    ...
    private class OnButtonClick : ICommand {
        ...
        public OnButtonClick(Action wtd) {
            instance = wtd;
        }

...
public static class Install {
    ...
    public static void Init(string[]? args = null) {
        ...
        if (string.IsNullOrEmpty(Cache)) {
            var hrip = Path.Combine(appdata, "HRtoVRChatLauncher");
            if (!Directory.Exists(hrip))
                Directory.CreateDirectory(hrip);
            Cache = hrip;
        }

        var hrp = Path.Combine(Cache, "HRtoVRChat");
    ...
    private static void UpdateProgressBar(int x) {
        RequestProgressBarUpdate.Invoke(x);
    }
    ...
    public static void InstallSoftware(Action? callback = null) {
        if (!IsUpdating) {
            ...
            using (var client = new WebClient()) {
                var outputFile = Path.Combine(Location, "HRtoVRChat.zip");
    ...
    public static string GetInstalledVersion() {
        if (File.Exists(Path.Combine(Location, "version.txt"))) {
            var file = Path.Combine(Location, "version.txt");
            if (File.Exists(file)) {
